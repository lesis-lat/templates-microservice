...
sub validate_yaml {
    ...
    for my $field (keys %text_fields) {
        if ( !( exists $vulnerability -> {$field} && ref $vulnerability -> {$field} eq 'HASH' ) ) {
            print "[ERROR] Missing or malformed field '$field' in $file\n";
            next;
        }
        for my $lang (keys %required_langs) {
            if ( !( exists $vulnerability -> {$field}{$lang} && defined $vulnerability -> {$field}{$lang} && $vulnerability -> {$field}{$lang} ne q{} ) ) {
    ...
    for my $field (keys %required_fields) {
        if ( !exists $vulnerability -> {$field} ) {
            print "[ERROR] Missing required field '$field' in $file\n";
        }
    }

    if ( exists $vulnerability -> {references} ) {
        if ( ref $vulnerability -> {references} ne 'ARRAY' ) {
